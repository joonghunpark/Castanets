...
std::string LocaleManager::GetSystemLocale() {
  /*
  char* str = NULL;
...
void LocaleManager::UpdateSystemLocale() {
  /*
  std::string lang = GetSystemLocale();
  if (lang.length() == 0) {
    LOG(ERROR) << "Language tag was invalid";
    return;
  }

  system_locales_.clear();
  while (true) {
    LOG(INFO) << "Processing language description: " << lang;
